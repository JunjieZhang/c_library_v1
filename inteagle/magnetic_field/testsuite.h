...
static void mavlink_test_magnetic_field(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_MAGNETIC_FIELD >= 256) {
            return;
        }
#endif
    ...
    mavlink_magnetic_field_t packet1, packet2;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           ...
           memset(MAVLINK_MSG_ID_MAGNETIC_FIELD_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_MAGNETIC_FIELD_MIN_LEN);
        }
#endif
