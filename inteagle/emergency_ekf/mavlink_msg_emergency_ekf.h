...
#define MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN 64
#define MAVLINK_MSG_ID_153_LEN 64
#define MAVLINK_MSG_ID_153_MIN_LEN 64
                               ...
                               uint64_t timestamp, float height_world, float vel_body_x, float vel_body_y, float vel_body_z, float q_w, float q_x, float q_y, float q_z, float reference_pressure, float phi, float theta, float psi, float reference_height, float terrain_bias)
{
    ...
    return mavlink_finalize_message(msg, system_id, component_id, MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_CRC);
}
                                   ...
                                   uint64_t timestamp,float height_world,float vel_body_x,float vel_body_y,float vel_body_z,float q_w,float q_x,float q_y,float q_z,float reference_pressure,float phi,float theta,float psi,float reference_height,float terrain_bias)
{
    ...
    msg->msgid = MAVLINK_MSG_ID_EMERGENCY_EKF;
    return mavlink_finalize_message_chan(msg, system_id, component_id, chan, MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_CRC);
}
...
static inline void mavlink_msg_emergency_ekf_send(mavlink_channel_t chan, uint64_t timestamp, float height_world, float vel_body_x, float vel_body_y, float vel_body_z, float q_w, float q_x, float q_y, float q_z, float reference_pressure, float phi, float theta, float psi, float reference_height, float terrain_bias)
{
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_EMERGENCY_EKF, buf, MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_CRC);
#else
    mavlink_emergency_ekf_t packet;
    packet.timestamp = timestamp;
    packet.height_world = height_world;
    ...
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_EMERGENCY_EKF, (const char *)&packet, MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_CRC);
#endif
}
 ...
 */
static inline void mavlink_msg_emergency_ekf_send_struct(mavlink_channel_t chan, const mavlink_emergency_ekf_t* emergency_ekf)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_emergency_ekf_send(chan, emergency_ekf->timestamp, emergency_ekf->height_world, emergency_ekf->vel_body_x, emergency_ekf->vel_body_y, emergency_ekf->vel_body_z, emergency_ekf->q_w, emergency_ekf->q_x, emergency_ekf->q_y, emergency_ekf->q_z, emergency_ekf->reference_pressure, emergency_ekf->phi, emergency_ekf->theta, emergency_ekf->psi, emergency_ekf->reference_height, emergency_ekf->terrain_bias);
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_EMERGENCY_EKF, (const char *)emergency_ekf, MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN, MAVLINK_MSG_ID_EMERGENCY_EKF_CRC);
#endif
}
...
#else
        uint8_t len = msg->len < MAVLINK_MSG_ID_EMERGENCY_EKF_LEN? msg->len : MAVLINK_MSG_ID_EMERGENCY_EKF_LEN;
        memset(emergency_ekf, 0, MAVLINK_MSG_ID_EMERGENCY_EKF_LEN);
    memcpy(emergency_ekf, _MAV_PAYLOAD(msg), len);
#endif
}
