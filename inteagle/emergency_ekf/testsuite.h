...
static void mavlink_test_emergency_ekf(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_EMERGENCY_EKF >= 256) {
            return;
        }
#endif
    ...
    mavlink_emergency_ekf_t packet_in = {
        93372036854775807ULL,73.0,101.0,129.0,157.0,185.0,213.0,241.0,269.0,297.0,325.0,353.0,381.0,409.0,437.0
    };
    mavlink_emergency_ekf_t packet1, packet2;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           ...
           memset(MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_EMERGENCY_EKF_MIN_LEN);
        }
#endif
